...
def build_body_content(
    content, breadcrumb, show_section, show_outer_wrapper, show_article
):
...
def generate_html(config_dict, template):
    ...
    # Open Graph
    html = html.replace(
        "{{OG_TITLE}}", config_dict.get("og_title", config_dict.get("title", ""))
    )
    html = html.replace(
        "{{OG_DESCRIPTION}}",
        config_dict.get("og_description", config_dict.get("description", "")),
    )
    ...
    # Twitter
    html = html.replace(
        "{{TWITTER_CARD}}", config_dict.get("twitter_card", "summary_large_image")
    )
    html = html.replace(
        "{{TWITTER_TITLE}}",
        config_dict.get("twitter_title", config_dict.get("title", "")),
    )
    html = html.replace(
        "{{TWITTER_DESCRIPTION}}",
        config_dict.get("twitter_description", config_dict.get("description", "")),
    )
    html = html.replace(
        "{{TWITTER_IMAGE}}", build_twitter_image(config_dict.get("twitter_image"))
    )

    # Profile and JSON-LD
    html = html.replace(
        "{{PROFILE_LINK}}", build_profile_link(config_dict.get("profile_link"))
    )
    ...
    html = html.replace(
        "{{CSS_IMPORTS}}", build_css_imports(config_dict.get("cssImports", ""))
    )
    html = html.replace("{{SCRIPTS}}", build_scripts(config_dict.get("scripts", [])))
    html = html.replace(
        "{{BODY_CLASS}}", build_body_class(config_dict.get("bodyClass", ""))
    )
    html = html.replace(
        "{{BREADCRUMB}}", build_breadcrumb(config_dict.get("showBreadcrumb", False))
    )
    html = html.replace(
        "{{LOADER}}", build_loader(config_dict.get("showLoader", False))
    )
    ...
    body_content = build_body_content(
        content, breadcrumb, show_section, show_outer_wrapper, show_article
    )
